function renderAllUsersData(data){
	...
	for(key in data){
		userTable+= "<tr><td>" + key + "</td><td>" + data[key]['username'] + "</td><td>" + data[key]['phone'] + 
			"</td><td>" + data[key]['email'] + "</td><td><button class='edit-btn' onclick='editUser(this);' value='Edit'>Edit</button>"+
			"<button type='button' class='btn btn-danger' data-toggle='modal' data-target='#myModal'>Delete</button></td></tr>";
	}
	userTable+= "</table>";
